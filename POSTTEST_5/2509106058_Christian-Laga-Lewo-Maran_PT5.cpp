...
void tampilkanStokSembako(BarangNode* head) {
    ...
    if (head == nullptr) { //bagian ini akan mengecek apakah linked list kosong
        cout << "Gudang kosong." << endl; //output jika linked list kosong
        return; //untuk keluar dari fungsi
    }

    BarangNode* temp = head; // membuat pointer bantu

    // memulai Traversal circular
    do { //memulai proses perulangan
        cout << "- " << temp->namaBarang << ": " << temp->stok << endl; // output nama dan stok barang
        temp = temp->next;
